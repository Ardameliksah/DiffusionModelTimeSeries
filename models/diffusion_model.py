...
class DiffusionModel(nn.Module):
    ...
    def forward(self, x_0: torch.Tensor, t: torch.Tensor = None) -> torch.Tensor:
        """
        Forward pass for training: predict x_0 directly from noisy sample (pred_x0 objective).
        ...
        Returns:
            (predicted_x0, x_0): both of shape (batch_size, channels, sequence_length)
        """
        ...
        x_t, _ = self.diffusion.q_sample(x_0, t)

        # Directly predict x_0 from noisy sample
        predicted_x0 = self.transformer(x_t, t)

        return predicted_x0, x_0
    
    def compute_loss(self, x_0: torch.Tensor, loss_type: str = "l1") -> torch.Tensor:
        """
        Compute training loss (pred_x0 objective, matching DiffusionTS / PaD-TS).
    ...
    def _predict_x_start(self, x_t: torch.Tensor, t: torch.Tensor, clip: bool = True):
        """Predict x_0 directly (pred_x0 objective), then derive noise for sampler consistency."""
        with torch.no_grad():
            x_start = self.transformer(x_t, t)
        if clip:
