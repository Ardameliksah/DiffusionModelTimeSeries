"""
Evaluation script for Image-UNet diffusion model.

Usage:
    python MyCode/src_image_unet/evaluate.py --embedding delay --checkpoint MyCode/output/checkpoints_unet_delay_mc64_nb2/best_model.pt
    python MyCode/src_image_unet/evaluate.py --embedding stft  --checkpoint MyCode/output/checkpoints_unet_stft_mc64_nb2/best_model.pt
"""
...
def evaluate(
    ...
    if output_dir is None:
        output_dir = str(Path(__file__).parent.parent / "output" / f"generated_samples_unet_{embedding}")
    Path(output_dir).mkdir(parents=True, exist_ok=True)

    print("=" * 80)
    print(f"EVALUATION — IMAGE-UNET ({embedding.upper()})")
    print("=" * 80)
    ...
    if checkpoint_path and Path(checkpoint_path).exists():
        ...
        if saved_cfg is not None:
            for key in ["model_channels", "num_blocks", "dropout"]:
                if key in saved_cfg:
                    setattr(config.model, key, saved_cfg[key])
            if "channel_mult" in saved_cfg:
                config.model.channel_mult = tuple(saved_cfg["channel_mult"])
            if "embedder_type" in saved_cfg:
                embedding = saved_cfg["embedder_type"]
                config.image.embedding_type = embedding
            print(f"   Arch restored: model_channels={config.model.model_channels}, "
                  f"num_blocks={config.model.num_blocks}, embedding={embedding}")
    ...
    if checkpoint is not None:
        ...
        print(f"   Image: ({model.img_channels}, {model.H}, {model.W}), "
    ...
    save_path = Path(output_dir) / f"comparison_unet_{embedding}.png"
    plot_real_vs_generated(real_train_batch, samples_np, save_path=str(save_path))

    # Save samples
    np.save(Path(output_dir) / f"samples_unet_{embedding}.npy", samples_np)

    # Save stats
    stats_path = Path(output_dir) / f"statistics_unet_{embedding}.txt"
    with open(stats_path, "w") as f:
        f.write(f"Mode: image_unet ({embedding})\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate Image-UNet diffusion model")
